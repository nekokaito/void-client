     ...
     return (
          ...
          <div className="dropdown dropdown-end border-white">
               <div tabIndex={0} role="button">
                    <img className=' w-12 h-12 object-cover rounded-full' src={`${userInfo?.photoURL || user?.photoURL || "/profile.png"}`} alt="" />
               </div>

               <ul tabIndex={0} className="dropdown-content flex flex-col gap-2 menu bg-base-100 rounded-box z-[1] w-52 p-2 shadow">
                    <li className='text-2xl font-bold btn bg-transparent hover:bg-transparent'>{userInfo.name || user?.displayName}</li>
